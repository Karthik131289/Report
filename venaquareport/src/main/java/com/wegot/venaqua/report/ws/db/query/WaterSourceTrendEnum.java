...
public enum WaterSourceTrendEnum {
    ...
    RAINWATER("Rain Water", 1, 2, "SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_rain_water_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_rain_water WHERE w2_rain_water.site_id=?) t2 ON t1.rain_water_id=t2.id AND (t1.dt >= ? and t1.dt< ? ) group by t1.dt order by t1.dt asc;"),
