...
public enum WaterSourceEnum {
    ...
    RAINWATER("Rain Water", 3, "SELECT t1.id, t1.rain_water_id, t1.day_total, t1.dt FROM w2_rain_water_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_rain_water WHERE w2_rain_water.site_id=?) t2 ON t1.rain_water_id=t2.id AND (t1.dt>=? AND t1.dt<? )"),
