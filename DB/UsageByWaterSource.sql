...
// final one
select t1.bwell_id, sum(t1.agg_total) as total, t2.cust_name, t2.pump_id from w2_bwell_day_total t1 inner join w2_pumps t2 on t2.site_id=4 and t2.id=t1.bwell_id and (t1.dt BETWEEN '2018-07-29 00:00:00' and '2018-08-04 23:59:59') group by t1.bwell_id;
...
/****** Sparkline chart ****/
// WTP
SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_wtp_component_day_total t1 JOIN (SELECT DISTINCT (wtp_id) FROM w2_wtp WHERE site_id=4) t2 ON t1.wtp_id=t2.wtp_id AND (t1.dt BETWEEN '2018-04-01 00:00:00' and '2018-04-31 23:59:59' ) group by t1.dt order by t1.dt asc;
// TANKER
SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_tanker_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_tankers WHERE w2_tankers.site_id=4) t2 ON t1.tanker_id=t2.id AND (t1.dt BETWEEN '2018-04-01 00:00:00' and '2018-04-31 23:59:59' ) group by t1.dt order by t1.dt asc;
// MUNICIPAL
SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_municipal_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_municipal WHERE w2_municipal.site_id=4) t2 ON t1.municipal_id=t2.id AND (t1.dt BETWEEN '2018-04-01 00:00:00' and '2018-04-31 23:59:59' ) group by t1.dt order by t1.dt asc;
// FLUSH
SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_flush_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_flush WHERE w2_flush.site_id=4) t2 ON t1.flush_id=t2.id AND (t1.dt BETWEEN '2018-04-01 00:00:00' and '2018-04-31 23:59:59' ) group by t1.dt order by t1.dt asc;
// DOMESTIC
SELECT sum(t1.day_total) as dayTotal, t1.dt FROM w2_domestic_day_total t1 JOIN (SELECT DISTINCT (id) FROM w2_domestic WHERE w2_domestic.site_id=4) t2 ON t1.domestic_id=t2.id AND (t1.dt BETWEEN '2018-04-01 00:00:00' and '2018-04-31 23:59:59' ) group by t1.dt order by t1.dt asc;
// GROUND
